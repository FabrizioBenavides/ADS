'====================================================================
' Class         : clsReporteClasificacion2Admin
' Title         : Grupo Benavides. Reporte Clasificacion 2
' Description   : Reporte Produccion Admin
...
Public Class clsReporteClasificacion2Admin
...
#Region "Document Business"
    ...
    Protected Overrides Sub initialize()
        Me.clsServicesObj = prjFotolabBusiness.Benavides.Fotolab.Business.clsReporteClasificacion2.SERVICIOS
    End Sub
    ...
    Protected Overrides Function strGenerarListado(ByVal strCmd As String) As String
        ...
        End If
        Dim arrayHeaders As Array = clsServicesObj.strConsultarListado("Headers", Nothing, strConnectionString, Me.strUsuarioNombre, intSelectedPage, _MAX_PER_PAGE)
		If Not IsNothing(arrayHeaders) And arrayHeaders.Length = 5 Then
			headers.Add("Cantidad Trabajos")

			Dim arrayValues As Array = DirectCast(arrayHeaders.GetValue(0), Array)
			headers.Add(arrayValues.GetValue(1))

			arrayValues = DirectCast(arrayHeaders.GetValue(1), Array)
			headers.Add(arrayValues.GetValue(1))

			arrayValues = DirectCast(arrayHeaders.GetValue(2), Array)
			headers.Add(arrayValues.GetValue(1))

			arrayValues = DirectCast(arrayHeaders.GetValue(3), Array)
			headers.Add(arrayValues.GetValue(1))

			arrayValues = DirectCast(arrayHeaders.GetValue(4), Array)
			headers.Add(arrayValues.GetValue(1))
		Else
			headers.Add("1")
			headers.Add("2")
		...
		htmlResult.Append("Reporte de Producción")
		...
		'htmlResult.Append("<table  width=100% cellSpacing=0 cellPadding=0  border=0 >	<tr class='trtitulos'><td class=rayita width=65%>&nbsp;</td><th  width=19%>Rollos&nbsp;</th><th  width=16%>Impresiones&nbsp;</th>	</tr></table>")
...
#Region "metodos de la interface PrintInterface"
    ...
    Protected Overrides Function getHeaders() As String
		...
		Dim widths As String() = {"40%", "10%", "10%", "10%", "10%", "10%", "10%"}
        Dim values As String()
        Dim arrayHeaders As Array = clsServicesObj.strConsultarListado("Headers", Nothing, strConnectionString, Me.strUsuarioNombre, 1, _MAX_PER_PAGE)
		Dim names(5) As String
		If Not IsNothing(arrayHeaders) And arrayHeaders.Length = 5 Then

			Dim arrayValues As Array = DirectCast(arrayHeaders.GetValue(0), Array)
			names.SetValue(arrayValues.GetValue(1), 0)

			arrayValues = DirectCast(arrayHeaders.GetValue(1), Array)
			names.SetValue(arrayValues.GetValue(1), 1)

			arrayValues = DirectCast(arrayHeaders.GetValue(2), Array)
			names.SetValue(arrayValues.GetValue(1), 2)

			arrayValues = DirectCast(arrayHeaders.GetValue(3), Array)
			names.SetValue(arrayValues.GetValue(1), 3)

			arrayValues = DirectCast(arrayHeaders.GetValue(4), Array)
			names.SetValue(arrayValues.GetValue(1), 4)
		Else
			names.SetValue("1", 0)
			names.SetValue("1", 0)
			names.SetValue("1", 0)
		...
		End If
		If Request("intUENId").Equals("2") Then
			Dim values1 As String() = {"Farmacia", "Cantidad Trabajos", CStr(names.GetValue(0)), CStr(names.GetValue(1)), CStr(names.GetValue(2)), CStr(names.GetValue(3)), CStr(names.GetValue(4))}
			values = values1
		Else
			Dim values2 As String() = {"Laboratorio", "Cantidad Trabajos", CStr(names.GetValue(0)), CStr(names.GetValue(1)), CStr(names.GetValue(2)), CStr(names.GetValue(3)), CStr(names.GetValue(4))}
			values = values2
		End If
    ...
    Protected Overrides Function getRow(ByVal aRow As Array, ByVal globalRowCounter As Integer) As String
		Dim colsToShow As Integer() = {0, 1, 2, 3, 4, 5, 6}
    ...
    Protected Overrides Function getTitle(ByVal currentPage As Integer, ByVal totalPages As Integer) As String
        ...
        Dim values1 As String() = {"Reporte de Producción", "", "HOJA: " + currentPage.ToString + " / " + totalPages.ToString}
        ...
        'code.Append("<table  width=100% cellSpacing=0 cellPadding=0  border=0 >	<tr class='trtitulos'><td class=rayita width=65%>&nbsp;</td><th  width=19%>Rollos&nbsp;</th><th  width=16%>Impresiones&nbsp;</th></tr></table>")
        Return code.ToString
    End Function
