<%@ Page Language="vb" AutoEventWireup="false" EnableSessionState="False" EnableViewState="False" Explicit="True" Trace="False" Strict="True" Codebehind="popClienteEspecial.aspx.vb" Inherits="com.isocraft.backbone.ccentral.PopClienteEspecial" codePage="28592" %>
<%@ Import Namespace="Benavides.POSAdmin.Commons" %>
<%  '====================================================================
    ' Page          : PopClienteEspecial.aspx
    ' Title         : Administracion POS y BackOffice
    ' Description   : Popup para selecionar el Cliente Especial
...
function cmdCerrar_onclick() {
	opener.document.forms[0].elements['<%=Request.QueryString("strCliente")%>'].value = 0;
	opener.document.forms[0].elements['<%=Request.QueryString("strClienteNombreId")%>'].value = "";
...
function ClosePopup(intValor,strNombre, intClienteEspecialId, strPrecioPromocion, strFechaInicio, strFechaFinal)
{
	opener.document.forms[0].elements['<%=Request.QueryString("strCliente")%>'].value = intValor;
	opener.document.forms[0].elements['<%=Request.QueryString("strClienteNombreId")%>'].value = strNombre;
	opener.document.forms[0].elements['intClienteEspecialId'].value = intClienteEspecialId;
...
<form name=frmPopClienteEspecial action=about:blank method=post>
